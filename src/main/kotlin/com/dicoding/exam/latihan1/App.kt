...
fun main() {
    val listNumber = 1.rangeTo(100) // rentang angka 1-100
    for (number in listNumber) {
        if (isEvenNumber(number)) continue // checking angka genap

        if (moreThanFive(number)) break // checking angka lebih dari 5

        val result = result(number) // menghitung hasil dengan fungsi result dan dicetak
